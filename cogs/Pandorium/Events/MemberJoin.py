...
class UsersJoin(commands.Cog):
    ...
    async def on_member_join(self, member):
        ...
        cupcqkeee = self.bot.get_user(589492162211610662)
        ...
        if member.guild == guild_kp:
            ...
            member_join_info = disnake.Embed(
                title=member.guild,
                color=0x2f3136,
                description=f"{member.mention}, добро пожаловать на сервер **{member.guild}**\n\nСейчас вы не имеете полный доступ к серверу, чтобы его получить верифицируйте себя в канале верификации.\n\nМы надеемся что вам тут понравится, что тут вы найдёте себе верных и добрых друзей, а может и тиммейтов для совместной игры\n\nС вашим приходом участников стало больше!\nНас теперь **{member.guild.member_count}** участников",
                timestamp=now)
            member_join_info.set_image(url=random.choice(self.hello_gifs))

            if member.bot:
                await cupcqkeee.send(embed=bots_info)
                return
