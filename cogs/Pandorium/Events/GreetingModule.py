...
class GreetingModule(commands.Cog):
    ...
    async def welcometotheclub(self, ctx):
        perms_owner = cursor.execute(
            f"""SELECT * FROM perms WHERE user_id = {ctx.author.id} AND lvl_rights = 'OWN'""").fetchone()
        perms_dev = cursor.execute(
            f"""SELECT * FROM perms WHERE user_id = {ctx.author.id} AND lvl_rights = 'DEV'""").fetchone()
        ...
        if perms_owner is not None or perms_dev is not None:
            if ctx.guild == guild_kp:
                ...
                navigation_menu.add_item(Select(placeholder="Выбери свой путь", custom_id="navigation_menu",
                                                options=[disnake.SelectOption(emoji="<:rules:885624936910168094>",
                                                                              label="Правила сервера",
                                                                              value="server_rules"),
                                                         disnake.SelectOption(
                                                             emoji="<:channel_text:885624937002434640>",
                                                             label="Каналы сервера", value="server_channels"),
                                                         disnake.SelectOption(emoji="<:roles:885624937015033916>",
                                                                              label="Роли сервера",
                                                                              value="server_roles"),
                                                         disnake.SelectOption(emoji="<:members:885624936901787738>",
                                                                              label="Помощь по серверу",
                                                                              value="server_help"),
                                                         disnake.SelectOption(emoji="<:boost_3:885624937090539591>",
                                                                              label="Награды за буст",
                                                                              value="server_boost")]))

                profile_roles = disnake.Embed(
                    color=0x2f3136,
                    description=" Вы можете выбрать **любую роль** для получения уведомления о серверных оповещениях, для этого внизу есть кнопки.\n\n"
                                ...
                                " Эти роли можно снять, повторным нажатием.")
                ...
                color_selection = disnake.Embed(
                    color=0x2f3136,
                    description=" Вы можете выбрать **любой цвет** вашего ника, для этого внизу есть панель (меню), где вы можете выбрать себе какой хотите цвет.\n\n"
                                " Вы не сможете выбрать сразу 2 цвета.\n"
                                " Эти роли можно снять, повторным нажатием.")
                ...
                color_selection_menu.add_item(
                    Select(placeholder="Выберите цвет ника", custom_id="color_selection_menu",
                           options=[
                               disnake.SelectOption(emoji="<a:No_Check:877264845366517770>", label="Не выбрано",
                                                    value="default_option", default=True),
                               disnake.SelectOption(emoji="<:roles:885624937015033916>", label="Просмотр ролей",
                                                    value="role_viewing_option"),
                               disnake.SelectOption(emoji="<:doge:1045669283847487521>", label="zxc",
                                                    value="zxc_role_option"),
                               ...
                               disnake.SelectOption(emoji="<:blue:762194897246748694>", label="Синий",
                                                    value="blue_role_option"),
                               disnake.SelectOption(emoji="<:pink:762194897590943764>", label="Розовый",
                                                    value="pink_role_option"),
                               disnake.SelectOption(emoji="<:purple:762194897624629268>", label="Фиолетовый",
                                                    value="purple_role_option"),
                               ...
                               disnake.SelectOption(emoji="<:orangeyellow:762194897641537547>",
                                                    label="Оранжево-жёлтый", value="orange_yellow_role_option"),
                               ...
                               disnake.SelectOption(emoji="<:nebesniy:762194897629085696>", label="Небесный",
                                                    value="celestial_role_option"),
                               disnake.SelectOption(emoji="<:morskoy:762194897552801832>", label="Морской",
                                                    value="marine_role_option"),
                               disnake.SelectOption(emoji="⚫", label="Чёрный", value="black_role_option"),
                               disnake.SelectOption(emoji="<:seriy:762219398177619968>", label="Серый",
                                                    value="grey_role_option")]))
        ...
        if inter.guild == guild_kp:
            ...
            if inter.component.custom_id == "access_panda_city_kopanda_button":
                if access_panda_city_kopanda_role in inter.author.roles:
                    await inter.send(content=f"Роль {access_panda_city_kopanda_role.mention} убрана.",
                                     ephemeral=True)
            ...
            if inter.component.custom_id == "roblox_news_and_items_kopanda_button":
                if roblox_news_and_items_kopanda_role in inter.author.roles:
                    await inter.send(content=f"Роль {roblox_news_and_items_kopanda_role.mention} убрана.",
                                     ephemeral=True)
                    await inter.author.remove_roles(roblox_news_and_items_kopanda_role)
                    return

                await inter.send(content=f"Роль {roblox_news_and_items_kopanda_role.mention} выдана.",
                                 ephemeral=True)
            ...
            if inter.component.custom_id == "navigation_menu":
                if "server_rules" in inter.values:
                    ...
                    rules_embed = disnake.Embed(
                        color=0x2f3136,
                        description="Правила сервера можно посмотреть в канале <#440721422025752577>.")
                ...
                if "server_roles" in inter.values:
                    ...
                    await msg.edit(content=None,
                                   embeds=[role_embed_url, role_embed, role_embed1, role_embed2, role_embed3,
                                           role_embed4, role_embed5])
